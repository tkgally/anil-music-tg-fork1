...
Final measurements (30 s cold runs at defaults, after review fixes — zero
console errors across the board):

| Prototype | RMS dBFS | level spread | centroid | energy >5 kHz | strong onsets/min | soft onsets/min |
|---|---|---|---|---|---|---|
| 01 Tides | −21.8 | 1.9 dB | 786 Hz | 0% | 0 | 0 |
| 02 Drift | −26.6 | ~8–10 dB* | 647 Hz | 0% | 0 | 0 |
| 03 Pulse | −22.9 | 1.9 dB | 381 Hz | 0% | 0 | 62 |
| 04 Dust | −18.5 | 6.2 dB | 1269 Hz | 0% | 64 | 78 |
| 05 Deep Work | −23.2 | 1.0 dB | 716 Hz | 0% | 0 | 0 |

\* Drift's spread depends on which phase of its 20–45 s swells a 30 s window
catches. Dust's strong onsets are its drums — lowpassed and velocity-soft;
Pulse's 62 *soft* onsets are pattern motion that never pokes above the bed.
Every prototype was also adversarially reviewed by a fresh agent (file://
contract, determinism, click-safety, hidden-tab scheduling, leaks, UI
wiring), and all confirmed findings were fixed — the commit history tells
that story per prototype.
...
## Keeping the GitHub Pages mirror in sync

GitHub Pages serves **`docs/`**, which is a 1:1 mirror of this folder (plus a
`.nojekyll`). **After any change to `studio-prototypes/`, run**
